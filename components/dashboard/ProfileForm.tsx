...
export default function ProfileForm({ profile }: { profile: Profile }) {
  ...
  async function resendVerification(){setError("");const response=await fetch("/api/auth/renvoyer-verification",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:profile.email})});const data=await response.json();if(response.ok)setMessage("E-mail de vérification envoyé");else setError(data.error||"Envoi impossible")}
  ...
  return <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[250px_1fr]">
    <aside className="space-y-4"><div className="card-sable p-5 text-center"><div className="mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-nihary-or-pale bg-cover bg-center" style={form.image?{backgroundImage:`url(${form.image})`}:undefined}>{!form.image&&<UserRound size={38}/>}</div><p className="mt-3 font-semibold">{form.name||"Votre profil"}</p><p className="text-xs text-nihary-gris">{profile.role}</p>{profile.emailVerified?<p className="mt-2 flex items-center justify-center gap-1 text-xs text-emerald-700"><ShieldCheck size={13}/>E-mail vérifié</p>:<button type="button" onClick={()=>void resendVerification()} className="mt-3 text-xs font-medium text-nihary-or hover:underline">Renvoyer l’e-mail de vérification</button>}</div><div className="card p-5"><div className="flex justify-between text-sm"><span>Complétude</span><strong>{completion}%</strong></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-nihary-sable"><div className="h-full bg-nihary-or" style={{width:`${completion}%`}}/></div><p className="mt-2 text-xs leading-5 text-nihary-gris">Les champs marqués * sont requis pour publier, enseigner ou rejoindre l’annuaire.</p></div></aside>
    <div className="space-y-6"><section className="card p-6"><span className="eyebrow">Identité professionnelle</span><div className="mt-4 grid gap-4 sm:grid-cols-2">{field("name","Nom complet","text",true)}{field("titreProfil","Fonction / titre","text",true,"Chercheur, consultante…")}{field("institution","Institution","text",true)}{field("discipline","Discipline principale","text",true)}{field("telephone","Téléphone","tel",true)}{field("villeProfil","Ville","text",true)}{field("pays","Pays","text",true)}{field("image","URL de l’avatar","url",false,"https://…")}</div><label className="mt-4 block text-sm font-medium text-nihary-brun">Biographie scientifique *<textarea className="input mt-1 min-h-36" required minLength={80} maxLength={3000} value={form.bio} onChange={event=>setForm({...form,bio:event.target.value})}/></label></section>
      ...
      <section className="card p-6"><span className="eyebrow">Sécurité</span><p className="mt-2 text-xs leading-5 text-nihary-gris">Confirmez votre mot de passe actuel avant d’en définir un nouveau.</p><div className="mt-4 grid gap-4 sm:grid-cols-2">{field("currentPassword","Mot de passe actuel","password",false,"Obligatoire en cas de changement")}{field("password","Nouveau mot de passe","password",false,"10 caractères minimum")}</div></section>
