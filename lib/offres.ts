...
export function opportunityToOffre(row: { slug:string;titre:string;organisation:string;type:string;payload:unknown;createdAt:Date }): Offre { const payload=row.payload&&typeof row.payload==="object"&&!Array.isArray(row.payload)?row.payload as Record<string,unknown>:{};const value=(key:string,fallback="N/A")=>typeof payload[key]==="string"&&payload[key]?payload[key] as string:fallback;return {slug:row.slug,titre:row.titre,organisation:row.organisation,type:(row.type in OFFRE_TYPE_LABELS?row.type:"EMPLOI") as OffreType,domaine:value("domaine"),lieu:value("lieu"),mode:value("mode","Présentiel") as Offre["mode"],datePublication:row.createdAt.toISOString().slice(0,10),dateLimite:value("dateLimite"),image:value("image","/uploads/magazine/magazine-economie-informelle.jpg"),resume:value("resume"),description:value("description"),missions:["N/A"],profil:["N/A"],competences:[value("domaine")],contrat:value("contrat"),duree:"N/A",tempsTravail:"N/A",remuneration:value("remuneration"),niveauEtudes:"N/A",experience:"N/A",financement:"N/A",debut:"N/A",langues:"N/A",avantages:["N/A"],candidature:["CV et lettre de motivation"],contact:value("contact")}; }
